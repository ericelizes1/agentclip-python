...
Use AgentClip's own browser tools by default:

- `browser_open` to launch Chromium in a fixed viewport
- `browser_navigate`, `browser_click`, `browser_type`, `browser_press_key`, and `browser_wait_for_text` to drive the flow
- `browser_screenshot` when the story is a discrete state
- `browser_start_recording` and `browser_stop_recording` when motion is the story
- `browser_close` when you're done

Then publish the captured files with `slideshow_create`, `slideshow_add_slide`, `slideshow_update_slide`, and `slideshow_set_summary`.
...
- **Faking it.** If the built-in browser tools fail, say so explicitly: *"Real evidence would require the AgentClip browser runtime to be available. Re-run `agentclip setup --force` and try again."* Never invent screenshots or recordings.
